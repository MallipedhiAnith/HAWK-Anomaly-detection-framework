...
def sparkline(data, anomaly_list, rca, 
              point=True, point_color='royalblue', point_marker='.',
              point_fill='royalblue', point_size=6, point_alpha=1.0,
              fill=True, fill_color='blue', fill_alpha=0.1,
              figsize=(1.4, 0.3), xlim=None, ylim=None, **kwargs):
    ...
    **kwargs : keyword arguments passed to matplotlib.pyplot.plot
    """
    if (data.name in anomaly_list) or (rca is not None):
        point_color, point_fill = 'red', 'red'
    ...
    plt.fill_between(range(plot_len), data, 0, color=fill_color, alpha=fill_alpha)
    ...
    msgImages = MIMEImage(bio.getvalue(), 'PNG')
    myimagelist.append(msgImages)

    return html

def create(data, anomaly_list=[], rca=None, **kwargs):
    ...
    **kwargs : keyword arguments to pass to sparkline function
    ...
    _df = pd.DataFrame()
    def _func(x):
        return sparkline(x, anomaly_list, rca, **kwargs)
    _df['sparkline'] = data.apply(_func, axis=1)
    return _df, myimagelist
